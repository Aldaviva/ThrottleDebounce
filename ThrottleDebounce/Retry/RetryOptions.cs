...
public interface IRetryOptions {
    ...
    /// <summary>Limits the attempts by total time, measured from the start of the first attempt to the end of the last attempt. Any attempts that would start after this duration has ended will not be run, and the most recent attempt exception will be thrown. Defaults to <c>null</c>, which means no upper limit on overall duration. If both <see cref="RetryOptions.MaxAttempts"/> and <see cref="RetryOptions.MaxOverallDuration"/> are specified, they will both be applied, so it will stop retrying after either enough attempts OR enough time has passed. This does not limit the duration of one individual attempt (which is unbounded), or the amount of waiting time between attempts (see <see cref="RetryOptions.Delay"/>), but rather the cumulative time allowed for all attempts together, from calling <c>Retrier.Attempt</c> to it returning or throwing. If you set this to a non-positive <see cref="TimeSpan"/>, it will make exactly one attempt.</summary>
